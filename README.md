...
This project automates the monitoring of SSL/TLS certificate expirations. The primary mechanism for this is a GitHub Actions workflow.
...
`domain` specifies the name of the step, which will be used in the interface and in error logs
`run` invokes the `npm run check-cert` script with two parameters:

- `www.example.se` is the hostname to check the certificate for
- `25` is the acceptable minimum number of days until certificate expiration.

If the certificate is valid for _less than the specified number of days_, or not valid at all, the step will result in an error and be reported in the workflow summary.
